...
# 0. 参数解析与路径配置
# ==========================================
parser = argparse.ArgumentParser(description="Amazon DCML Dataset Builder (Version 3)")
...
TREATMENT_PATH = os.path.join(BASE_DIR, f"item_multimodal_scalars/{DATASET_NAME}_item_multimodal_scalars_merged_full_2.parquet")
...
log_filename = f"dataset_builder_{DATASET_NAME}_v3_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
def main():
    try:
        logger.info("="*80)
        logger.info(f">>> 开始构建亚马逊 {DATASET_NAME} DCML 数据集 (v2)")
        logger.info("="*80)
        ...
        for name, data in zip(['Table_Master', 'Set_A', 'Set_B', 'Set_C'], [master_df, set_A, set_B, set_C]):
            # ⚠️ 这里修改了输出名称，加上了 _3
            out_path = os.path.join(SAVE_DIR, f"{DATASET_NAME}_DCML_{name}_3.parquet")
            data.to_parquet(out_path, index=False)
            logger.info(f"   - 已保存 {name} -> {out_path}")

        logger.info("="*80)
        logger.info(f"🎉 亚马逊数据集 ({DATASET_NAME}) DCML v3 构建成功！")
...
if __name__ == "__main__":
    ...
    #.  python /home/xzhe162/wh_workspace/casual/model/M_amazon/MCRE/dataset_builder_amazon_app.py