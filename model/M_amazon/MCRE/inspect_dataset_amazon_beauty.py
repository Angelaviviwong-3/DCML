...
# 0. 参数解析与路径配置
# ==========================================
parser = argparse.ArgumentParser(description="Inspect DCML Dataset for Amazon Beauty")
# 默认使用 amazon_beauty
...
# ⚠️ 注意这里指向 Beauty 数据集的尾缀 _1
SET_B_PATH = os.path.join(DATA_DIR, f"{DATASET_NAME}_DCML_Set_B_1.parquet")
SET_C_PATH = os.path.join(DATA_DIR, f"{DATASET_NAME}_DCML_Set_C_1.parquet")
...
if __name__ == "__main__":
    print(f">>> 正在加载亚马逊 ({DATASET_NAME}) DCML 实验数据集...")
    ...
    if os.path.exists(SET_B_PATH) and os.path.exists(SET_C_PATH):
        ...
        print(f"❌ 找不到数据文件。请检查路径或后缀是否为 _1.parquet: \n{SET_B_PATH}")
        
        #.  python /home/xzhe162/wh_workspace/casual/model/M_amazon/MCRE/inspect_dataset_amazon_beauty.py