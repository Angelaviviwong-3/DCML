...
# ⚠️ 核心修改：直接读取已经跑完的 3 个切片 CSV 文件
# 匹配规则: amazon_beauty_item_multimodal_scalars_full_2_*.csv
PARTIAL_FILES_PATTERN = os.path.join(SAVE_DIR, f"{DATASET_NAME}_item_multimodal_scalars_full_2_*.csv")

# 最终合并后输出的全量纯净版
MERGED_OUTPUT_FILE = os.path.join(SAVE_DIR, f"{DATASET_NAME}_item_multimodal_scalars_merged_full.parquet")

# 绘图输出路径
RAW_MACRO_PLOT = os.path.join(SAVE_DIR, f"{DATASET_NAME}_dist_01_macro_raw_full.png")
CALIB_MACRO_PLOT = os.path.join(SAVE_DIR, f"{DATASET_NAME}_dist_02_macro_calib_full.png")
CALIB_MKT_ATOMIC_PLOT = os.path.join(SAVE_DIR, f"{DATASET_NAME}_dist_03_atomic_mkt_calib_full.png")
CALIB_VIS_ATOMIC_PLOT = os.path.join(SAVE_DIR, f"{DATASET_NAME}_dist_04_atomic_vis_calib_full.png")

# 日志配置
log_filename = f"merge_ecdf_{DATASET_NAME}_full_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
logging.basicConfig(
    level=logging.INFO,
...
def main():
    ...
    # --- A. 合并切片文件 ---
    part_files = glob.glob(PARTIAL_FILES_PATTERN)
    # 因为存在 checkpoint 干扰，我们要过滤掉以 checkpoint 开头的文件
    part_files = [f for f in part_files if "checkpoint" not in os.path.basename(f)]
    
    if not part_files:
        logger.error(f"❌ 找不到任何分片文件，请检查路径: {PARTIAL_FILES_PATTERN}")
        return

    logger.info(f">>> 找到 {len(part_files)} 个数据分片，准备合并...")
    df_list = []
    ...
    for col in cols_to_calibrate:
        if col in df_merged.columns:
            # 无论之前切片里有没有 calibrated，这里全部基于全表重算一次以保证严格一致性
...
if __name__ == "__main__":
 ...
 #.  python /home/xzhe162/wh_workspace/casual/model/M_amazon/MCRE/merge_and_check_beauty.py