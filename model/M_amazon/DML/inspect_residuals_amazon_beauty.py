...
# ==========================================
# 0. 路径配置 (对接 Beauty 数据集的 Final 残差文件)
# ==========================================
DATASET_NAME = "amazon_beauty"
RES_PATH = f"/home/xzhe162/wh_workspace/casual/processed_data/{DATASET_NAME}/DML_Results/{DATASET_NAME}_DML_Residuals_Final.parquet"
...
def main():
    ...
    if not os.path.exists(RES_PATH):
        print(f"❌ 找不到文件: {RES_PATH}。请检查 DML 训练脚本是否成功运行。")
        return

    print(f"正在加载数据... 请稍候...")
    df_res = pd.read_parquet(RES_PATH)
    n_samples = len(df_res)
    print(f"✅ 数据加载完成，样本量: {n_samples:,} 行")
    ...
    if null_counts.max() > 0:
        ...
        print(f"\n✅ [1/4] 缺失值检查: 完美，全表 {n_samples:,} 行无任何 NaN！")
    ...
    print("   * 理论期望: 均值 (mean) 应极度趋近于 0 (例如 e-16 或 e-05 级别)，标准差 (std) 反映了扣除干扰后的净干预方差。")
    print("-" * 90)
    ...
    print("\n" + "="*100)
    print("🎉 残差诊断完成！如果 [Mean_Check] 全部为 OK，且相关矩阵存在强耦合 (>0.1)，请开启 Stage III：因果估计。")
    print("="*100)

if __name__ == "__main__":
    main()
    
   #. python /home/xzhe162/wh_workspace/casual/model/M_amazon/DML/inspect_residuals_amazon_beauty.py