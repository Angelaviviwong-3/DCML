...
# 日志配置 (版本 v1)
log_filename = f"causal_inference_beauty_v1_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
logging.basicConfig(
    level=logging.INFO,
...
def main():
    logger.info("="*80)
    logger.info(f">>> [DCML Stage III] 亚马逊 {DATASET_NAME} 最终因果效应估计 (V1)")
    logger.info(">>> [目标]: 包含 Std_Error 字段，输出全量 ATE/CATE 结果")
    logger.info("="*80)

    # 1. 载入残差数据
    logger.info(f">>> 1. 正在载入残差矩阵 (样本量: 656万)...")
    ...
    # ---------------------------------------------------------
    logger.info("\n>>> 2. 执行 Multivariate Gram-Schmidt 正交化投影 (解耦双系统)...")
    ...
    for target in tqdm(int_macro, desc="Macro Orthogonalization"):
        # 核心投影：去除 System 2 中能被 System 1 解释的部分
    ...
    def run_causal_engine(treatment_cols, res_label):
        ...
        # B. CATE 分层条件处理效应 (按卷入度 H_level)
        logger.info(f"   - 正在分层计算 {res_label} 组 CATE...")
        h_levels = sorted(df['H_level'].unique())
        for h in tqdm(h_levels, desc=f"{res_label} CATE Regression"):
    ...
    # 执行计算并保存
    # ---------------------------------------------------------
    macro_final_results = run_causal_engine(T_star_macro, "Macro")
    atomic_final_results = run_causal_engine(T_star_atomic, "Atomic")

    df_output = pd.DataFrame(macro_final_results + atomic_final_results)
    
    # 后缀 v1
    ate_path = os.path.join(SAVE_DIR, f"{DATASET_NAME}_Final_ATE_v1.csv")
    cate_path = os.path.join(SAVE_DIR, f"{DATASET_NAME}_Final_CATE_v1.csv")
    
    # 拆分保存
    df_output[df_output['H_level'] == 'All_ATE'].to_csv(ate_path, index=False)
    df_output[df_output['H_level'] != 'All_ATE'].to_csv(cate_path, index=False)

    logger.info("="*80)
    logger.info(f"🎉 亚马逊 Beauty 因果估计 V1 完成！")
    logger.info(f"💾 ATE 结果路径: {ate_path}")
    logger.info(f"💾 CATE 结果路径: {cate_path}")
    logger.info("="*80)
