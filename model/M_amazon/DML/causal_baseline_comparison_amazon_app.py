...
# 读取构建好的推理集 Set C
DATA_PATH = os.path.join(BASE_DIR, f"build_dataset/{DATASET_NAME}_DCML_Set_C_3.parquet")
...
log_filename = f"baseline_comparison_{DATASET_NAME}_v2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
def main():
    logger.info("="*80)
    logger.info(f">>> [Section 5.3.2] 运行传统基线估计器对比 (Naive OLS & IPW) - {DATASET_NAME}")
    logger.info("="*80)
    ...
    if not os.path.exists(DATA_PATH):
        ...
        return
    logger.info(f">>> 正在加载数据集 (2100万样本)...")
    df = pd.read_parquet(DATA_PATH)
    
    Y_cols = ['y_click', 'y_cart', 'y_purchase']
    
    # 定义 Treatment 组 (适配亚马逊带有 _calibrated 后缀的列)
    T_macro = [
    ...
    T_atomic = [
        ...
        'T_int_fac_calibrated' # 原子层面复用事实密度
    ]
    ...
    # ---------------------------------------------------------
    # Baseline 1: Naive OLS (完全忽略混淆变量 X)
    # ---------------------------------------------------------
    logger.info("\n>>> 1. 正在运行 Naive OLS (模拟未纠偏的原始相关性)...")
    ...
    # ---------------------------------------------------------
    logger.info("\n>>> 2. 正在运行 IPW (验证连续倾向得分 GPS)...")
    
    # 获取唯一的处理变量集合
    all_unique_t = list(set(T_macro + T_atomic))
    
    # 注意：IPW 估计 2100万行计算量极大，我们使用高效的 GPS 计算法
    for t_col in tqdm(all_unique_t, desc="Estimating IPW/GPS Weights"):
        # 1. 拟合 E[T|X] 使用轻量级 LGBM
        model_t = lgb.LGBMRegressor(n_estimators=50, num_leaves=31, n_jobs=-1, verbose=-1)
        model_t.fit(df[X_cols], df[t_col])
        t_hat = model_t.predict(df[X_cols])
        
        # 2. 计算广义倾向得分权重 (Generalized Propensity Score)
        # 假设 T|X 服从正态分布：W = f(T) / f(T|X)
    ...
    out_path = os.path.join(SAVE_DIR, f"{DATASET_NAME}_Baseline_Comparison_v2.csv")
    df_comparison.to_csv(out_path, index=False)

    logger.info("\n" + "="*80)
    logger.info("🎉 基线对照实验圆满完成！")
    logger.info(f"💾 结果已保存至: {out_path}")
    logger.info(f"📊 记录总数: {len(df_comparison)} 条")
    logger.info("========================================================\n")

if __name__ == "__main__":
    main()
    
     #. python /home/xzhe162/wh_workspace/casual/model/M_amazon/DML/causal_baseline_comparison_amazon_app.py