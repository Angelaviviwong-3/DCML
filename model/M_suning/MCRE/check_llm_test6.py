...
# 输入文件
INPUT_FILE = os.path.join(SAVE_DIR, "item_multimodal_scalars_test6.parquet")

# 输出图像路径
RAW_MACRO_PLOT = os.path.join(SAVE_DIR, "dist_01_macro_raw_test6.png")
CALIB_MACRO_PLOT = os.path.join(SAVE_DIR, "dist_02_macro_calib_test6.png")
CALIB_MKT_ATOMIC_PLOT = os.path.join(SAVE_DIR, "dist_03_atomic_mkt_calib_test6.png")
CALIB_VIS_ATOMIC_PLOT = os.path.join(SAVE_DIR, "dist_04_atomic_vis_calib_test6.png")

# 日志配置
log_filename = f"ecdf_plotting_test6_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
logging.basicConfig(
    level=logging.INFO,
...
def main():
    if not os.path.exists(INPUT_FILE):
        logger.error(f"❌ 找不到测试数据文件: {INPUT_FILE}。请先运行 test6 数据抽取脚本！")
        return
...
if __name__ == "__main__":
    ...
    #. python /home/xzhe162/wh_workspace/casual/model/MCRE/check_llm_test6.py