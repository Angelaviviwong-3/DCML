...
Read meta_*.jsonl and download the high-resolution main image for each ASIN.
Supports dataset names and configurable logging and output paths.
"""
...
# ----------------------------- Basic configuration -----------------------------
DATASET_NAME = "amazon_appliances"  # Dataset identifier
...
# ----------------------------- Path configuration -----------------------------
DEFAULT_JSONL = str(_dcml_paths.project_path('raw_data/')) + "/"
DEFAULT_OUTPUT = str(_dcml_paths.project_path('processed_data/')) + "/"
...
def download_one(task, output_dir, retries=DEFAULT_RETRIES):
    ...
    # Check for an existing file before downloading
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Download Amazon main product images concurrently")
