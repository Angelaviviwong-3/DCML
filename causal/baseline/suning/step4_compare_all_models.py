#!/usr/bin/env python3
"""Audit and compare the independent DCML recommender with Suning baselines."""
...
def validate_dcml_bundle(root: Path, stage: str, candidates: dict, metadata: dict, prediction_file: Path) -> list[str]:
    ...
    if not as_bool(metadata.get("multi_task_training", False)):
        errors.append("multi_task_training is not true")
    return errors
...
def main() -> None:
    ...
    baseline_dir = _dcml_paths.workspace_path(root, 'results_of_comparison') / "suning" / "baseline_data"
    ...
    for stage in STAGES:
        ...
        errors = []
        if stage_metadata.get("user_sampling_method") != "all_stage_warm_start_users":
            errors.append("evaluation does not include all eligible stage-warm users")
        if int(stage_metadata.get("evaluated_users", -1)) != int(stage_metadata.get("stage_warm_start_eligible_users", -2)):
            errors.append("evaluated users differ from stage-warm eligible users")
    ...
    if not any(row["Model"] == "DCML (Ours)" for row in missing_rows):
        ...
        if not required_prior_columns.issubset(prior.columns):
            ...
            if len(h0_semantic) != len(STAGES) or not h0_semantic["Fallback_to_ATE"].map(as_bool).all():
                invalid_rows.append({"Model": "DCML (Ours)", "Stage": "ALL", "Reason": "unsupported_h0_semantic_prior", "Detail": "H=0 semantic prior did not fall back to ATE"})
            else:
                ate_semantic = prior[
                    (prior["Prior_Sample"].astype(str) == "Full Set B")
        ...
        if len(stability) != 36 or not stability["Gate_Use"].astype(str).str.contains("not used", regex=False).all():
            invalid_rows.append({"Model": "DCML (Ours)", "Stage": "ALL", "Reason": "invalid_stability_audit", "Detail": f"stability rows={len(stability)}, expected 36"})
        ...
        for record in dcml_summary.to_dict("records"):
            ...
            record.update(
                {
                    "Training_Target": "multi-task y_click|y_cart|y_purchase",
    ...
    quality_errors = []
    if len(table) != 27:
        quality_errors.append(f"Table rows={len(table)}, expected 27")
    if len(baseline_audit) != 24 or (not baseline_audit.empty and not baseline_audit["Bundle_Valid"].all()):
        quality_errors.append("Not all 24 baseline prediction bundles passed audit")
    if len(dcml_audit) != 3 or (not dcml_audit.empty and not dcml_audit["Audit_Passed"].all()):
        quality_errors.append("Not all three independent DCML prediction bundles passed audit")
    ...
    if not users.empty and not quality_errors:
        ...
        if not quality_errors:
            ...
            if len(paired) != 96 or (paired["Paired_Users"] <= 0).any():
                quality_errors.append(f"Paired test rows={len(paired)}, expected 96")
    ...
    if candidate_sensitivity_path.is_file() and candidate_sensitivity_metadata_path.is_file():
        ...
        candidate_sensitivity_complete = bool(
            len(sensitivity) == 297
            and int(sensitivity_metadata.get("actual_rows", -1)) == 297
    ...
    if training_seed_summary_path.is_file() and not table.empty:
        ...
        if len(seed_summary) == 27:
            table = table.merge(seed_summary, on=["Stage", "Model"], how="left", validate="one_to_one")
        else:
            quality_errors.append(f"Training seed summary rows={len(seed_summary)}, expected 27")
    ...
    write_json(
        {
            ...
            "rows": int(len(users)),
            "expected_model_stage_bundles": 27,
    ...
    write_json(
        {
            ...
            "expected_table_rows": 27,
            "actual_table_rows": len(table),
            "expected_baseline_bundles": 24,
            "actual_baseline_bundles": len(baseline_audit),
            "expected_independent_dcml_bundles": 3,
            "actual_independent_dcml_bundles": len(dcml_audit),
            "expected_paired_tests": 96,
            "actual_paired_tests": len(paired),
            "dcml_independent_model_audit_passed": bool(len(dcml_audit) == 3 and dcml_audit["Audit_Passed"].all()),
