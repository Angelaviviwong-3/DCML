#!/usr/bin/env python3
"""Train the independent end-to-end multi-task DCML recommender."""
...
from dcml_recommender_core import (  # noqa: E402
    DCMLRecommender,
    bpr_loss,
    funnel_order_loss,
)
...
def split_path(root: Path, split: str) -> Path:
    filename = f"DCML_{split}_{DATA_VERSION}.parquet"
...
def load_split(root: Path, split: str) -> tuple[pd.DataFrame, Path]:
    ...
    certified_items, _, _ = load_certification(root, "suning")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--funnel-loss-weight", type=float, default=0.02)
    ...
    parser.add_argument("--preflight-only", action="store_true", help="Audit inputs and item-category coverage, then stop.")
...
def history_level_cuts(reference: pd.DataFrame) -> np.ndarray:
    values = pd.to_numeric(reference["user_events_pre_log"], errors="coerce").replace([np.inf, -np.inf], np.nan)
...
def event_history_levels(frame: pd.DataFrame, cuts: np.ndarray) -> np.ndarray:
    values = pd.to_numeric(frame["user_events_pre_log"], errors="coerce").fillna(0.0).to_numpy(float)
...
def end_of_window_user_levels(
    ...
    """Return the involvement state after all events observed through a cutoff."""
    levels = np.zeros(len(user2id), dtype=np.int64)
    counts = history.assign(user_id=history["user_id"].astype(str)).groupby("user_id", sort=False).size()
...
def user_state_audit_rows(
    ...
    for stage, candidates in candidates_by_stage.items():
        ...
        row = {
            ...
            "State_Definition": "log1p(all events observed through cutoff), discretized by prehistory tertile cuts",
        }
...
def load_item_categories(root: Path, item2id: dict[str, int]) -> tuple[np.ndarray, dict]:
    path = first_existing(
        [
            _dcml_paths.workspace_path(root, 'processed_data') / "suning" / "item_feature&Confounder_price" / "item_text_price_matrix.parquet",
            _dcml_paths.workspace_path(root, 'processed_data') / "suning" / "item_feature&Confounder_price" / "item_text_price_matrix.csv",
        ]
    )
    if path.suffix == ".parquet":
        try:
            ...
            items = pd.read_csv(path.with_suffix(".csv"), encoding="utf-8-sig", encoding_errors="replace")
    else:
        items = pd.read_csv(path, encoding="utf-8-sig", encoding_errors="replace")
    if "category" in items:
        category = items["category"].fillna("Unknown").astype(str)
    elif "cat_name" in items:
        category = items["cat_name"].fillna("Unknown").astype(str)
    else:
        text = items.get("text_W", pd.Series("Unknown", index=items.index)).fillna("").astype(str)
        category = text.str.extract(r"(?:Category|\u7c7b\u76ee)[:\uff1a]\s*([^|]+)")[0].fillna("Unknown").str.strip()
    lookup = {normalize_item_id(item): str(cat) for item, cat in zip(items["item_id"], category)}
    output = np.asarray(["Unknown"] * len(item2id), dtype=object)
    missing_items = []
    for raw_item, index in item2id.items():
        normalized = normalize_item_id(raw_item)
        if normalized not in lookup:
            missing_items.append(raw_item)
            continue
        output[int(index)] = lookup[normalized]
    audit = {
        "script_version": SCRIPT_VERSION,
        "category_source_file": str(path),
        "category_source_sha256": sha256_file(path),
        ...
        "requirement": "every B/C item must map to the same item-category source used to construct T_int_sem",
    }
    return output, audit
...
def semantic_history(
    ...
    category_clicks: dict[tuple[int, str], int] = defaultdict(int)
    positive = frame.loc[frame["y_click"].fillna(0).astype(int) == 1, ["user_id", "item_id"]]
    for row in positive.itertuples(index=False):
        ...
        totals[user] += 1
        category_clicks[(user, str(item_categories[item]))] += 1
    return totals, category_clicks
...
def semantic_alignment(
    ...
    item_categories: np.ndarray,
    totals: np.ndarray,
    category_clicks: dict[tuple[int, str], int],
) -> float:
    total = int(totals[user])
    if total <= 0:
        return 0.0
    return float(np.clip(category_clicks.get((user, str(item_categories[item])), 0) / total, 0.0, 1.0))
...
def fit_training_residuals(
    ...
    for index, (component, column) in enumerate(zip(MACRO_COMPONENTS, MACRO_COLUMNS)):
        ...
        target = pd.to_numeric(nuisance_frame[column], errors="coerce").fillna(0.0).to_numpy(float)
        model.fit(x_train, target)
        observed = pd.to_numeric(effect_frame[column], errors="coerce").fillna(0.0).to_numpy(float)
        residuals[f"T::{component}"] = observed - model.predict(x_effect)
...
def estimate_causal_prior(
    ...
    conformity = np.column_stack([residuals[f"T::{component}"][mask] for component in MACRO_COMPONENTS[:3]])
    interest = np.column_stack([residuals[f"T::{component}"][mask] for component in MACRO_COMPONENTS[3:]])
    projection_design = np.column_stack([np.ones(mask.sum()), conformity])
    ...
    h_values = np.asarray(h_values_all, dtype=int)[mask]
    ...
    groups = [("ATE", np.ones(mask.sum(), dtype=bool))]
    if include_cate:
        groups.extend((str(level), h_values == level) for level in range(4))
    for stage in STAGES:
        y = residuals[f"Y::{stage}"][mask]
        ate_scoring = None
...
def candidate_causal_scores(
    ...
    semantic_category_clicks: dict[tuple[int, str], int],
    content_columns: list[str],
) -> np.ndarray:
    ...
    for item in items:
        treatment = {
            ...
            "T_int_sem": semantic_alignment(user, item, item_categories, semantic_totals, semantic_category_clicks),
        }
...
def main() -> None:
    ...
    _, certification_summary, _ = load_certification(root, "suning")
    device = device_from_arg(args.device)
    seed_offset = int(os.environ.get("SUNING_TRAINING_SEED_OFFSET", "0"))
    ...
    schema = read_json(first_existing(result_input_paths(root, f"DML_feature_schema_{SCRIPT_VERSION}.json", version=SCRIPT_VERSION)))
    content_columns = [column for column in schema["treatments"] if column != "T_int_sem"]
    missing = sorted((set(MACRO_COLUMNS) - set(set_a.columns)) | (set(MACRO_COLUMNS) - set(set_b.columns)))
    if missing:
        raise ValueError(f"Missing required DCML treatment columns: {missing}")
    ...
    if category_audit["missing_items"]:
        raise RuntimeError(
            "The item-category source is inconsistent with the saved DCML A/B/C data: "
            f"{category_audit['missing_items']} of {category_audit['required_items']} item IDs are missing from "
            f"{category_audit['category_source_file']}. Restore the complete item_text_price_matrix used by "
            "dataset_builder.py before training. See DCML_Item_Category_Audit_20260717.json."
        )
    if args.preflight_only:
        logger.info("DCML recommendation preflight passed; category coverage is complete")
        return
    ...
    residuals = fit_training_residuals(
        set_a,
        set_b,
        schema.get("confounder_whitelist", schema.get("confounders", [])),
        seed,
    )
    ...
    for index, stage in enumerate(STAGES):
        ...
        if not validation_candidates[stage]:
            raise RuntimeError(f"No warm validation candidates for {stage}")
        write_json(
    ...
    for stage in STAGES:
        ...
        metadata = {
            ...
            "architecture": (
                "multi-task collaborative representation + MCRE content encoder + support-aware training-only "
                "DML causal-informed score"
            ),
            ...
            "user_state_definition": "all Set A+B events observed through the Set C cutoff; no Set C table access",
    ...
    write_json(
        {
            ...
            "prediction_semantic_missing_encoding": (
                "zero is a prediction-only encoding when no click history exists; it is not an estimable causal T_int_sem value"
            ),
            ...
            "user_state_definition": "all events observed through the applicable cutoff, including Set A history",
