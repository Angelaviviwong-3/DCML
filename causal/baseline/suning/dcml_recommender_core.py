#!/usr/bin/env python3
"""Core model for the independent Suning DCML recommender."""
...
class DCMLRecommender(nn.Module):
    """Multi-task collaborative-content model with an internal DCML scoring head."""
...
def funnel_order_loss(stage_scores: list[torch.Tensor]) -> torch.Tensor:
    """Penalize violations of P(purchase) <= P(cart) <= P(click)."""
    click, cart, purchase = [torch.sigmoid(score) for score in stage_scores]
    return (F.relu(cart - click) + F.relu(purchase - cart)).mean()
