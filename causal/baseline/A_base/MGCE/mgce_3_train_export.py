#!/usr/bin/env python3
"""Train one MGCE model per Suning funnel outcome and export audited scores."""
...
def main() -> None:
    ...
    image = np.load(cache / "image_feat_20260717.npy")
    diagnostics = {"script_version": "20260717", "stage_specific_training": True, "stages": {}}
    for stage in STAGES:
        seed = stage_seed(stage, 71)
        rng = np.random.default_rng(seed)
        data = np.loadtxt(cache / f"train_{stage}_20260717.txt", dtype=np.int64).reshape(-1, 2)
        popularity = np.load(cache / f"pop_feat_{stage}_20260717.npy")
        ...
        session.close()
    write_json(diagnostics, baseline_data_dir(root) / "MGCE_training_diagnostics_20260717.json", indent=2)
