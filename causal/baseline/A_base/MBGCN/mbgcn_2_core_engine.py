#!/usr/bin/env python3
"""Independent stage-graph encoder used by the 20260717 MBGCN rerun."""
...
class TrainDataset(Dataset):
    def __init__(self, cache_path: str, relations: list[str]):
        ...
        self.user_num, self.item_num = map(int, (self.path / "data_size_20260717.txt").read_text(encoding="utf-8").split())
        self.checkins = np.loadtxt(self.path / f"{relations[0]}_20260717.txt", dtype=np.int64).reshape(-1, 2)
    ...
    def _adjacency(self, relation: str) -> torch.Tensor:
        pairs = np.loadtxt(self.path / f"{relation}_20260717.txt", dtype=np.int64).reshape(-1, 2)
        users = pairs[:, 0]
    ...
    def _load_samples(self) -> None:
        path = self.path / "sample_file" / f"sample_{self.sample_index % 5}_20260717.txt"
