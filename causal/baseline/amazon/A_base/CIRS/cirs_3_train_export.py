#!/usr/bin/env python3
"""Train one CIRS model per Amazon Purchase outcome and export audited scores."""
...
def main() -> None:
    ...
    for stage in STAGES:
        ...
        data = pd.read_csv(cache / f"train_with_time_{stage}_20260718.txt", sep=r"\s+", header=None, names=["user", "item", "slot"])
        ...
        popularity = np.load(cache / f"item_popularity_{stage}_20260718.npy")
        ...
        session.close()
    write_json(diagnostics, baseline_data_dir(root) / "CIRS_training_diagnostics_20260718.json", indent=2)
