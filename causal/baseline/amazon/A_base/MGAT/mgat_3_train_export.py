#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Train one MGAT model for each Amazon Purchase outcome."""
...
def train_stage(root: Path, stage: str, logger, device: torch.device, features, user_num: int, item_num: int) -> dict:
    ...
    edges = np.load(cache / f"train_{stage}_20260718.npy")
    adjacency = np.load(cache / f"adjacency_{stage}_20260718.npy", allow_pickle=True).item()
    if not len(edges):
...
def main() -> None:
    ...
    cache = cache_dir(root, "mgat_cache")
    with open(cache / "data_size_20260718.txt", "r", encoding="utf-8") as handle:
        user_num, item_num = map(int, handle.read().split())
    features = [np.load(cache / f"Feature{name}_normal_20260718.npy") for name in ("Video", "Text", "Audio")]
