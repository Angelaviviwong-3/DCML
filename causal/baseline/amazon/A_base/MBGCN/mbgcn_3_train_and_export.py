#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Train the independently supervised Amazon Purchase-only MBGCN model."""
...
def train_stage(root: Path, stage: str, logger, device: torch.device) -> dict:
    ...
    predictions = {}
    with torch.no_grad():
        final_user, final_item = model.embeddings()
        for user, data in tqdm(candidates.items(), desc=f"MBGCN export {stage}", leave=False):
            items = candidate_items(data)
            item_tensor = torch.as_tensor(items, dtype=torch.long, device=device)
            scores = final_user[int(user)] @ final_item[item_tensor].t()
            score_values = scores.detach().cpu().tolist()
            predictions[user] = {str(item): float(score) for item, score in zip(items, score_values)}
    metadata = export_prediction_bundle(
        ...
        training_protocol="independent Purchase-only BPR graph model on Set B",
        extra_metadata={
            "seed": seed,
            "first_epoch_loss": losses[0],
            "last_epoch_loss": losses[-1],
            "export_embedding_mode": "single final full-graph propagation reused for every candidate user",
        },
    )
    return metadata
