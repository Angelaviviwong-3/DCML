#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Build independent stage-specific MBGCN caches for Amazon Purchase-only."""
...
def main() -> None:
    ...
    for stage in STAGES:
        ...
        (out_dir / "data_size_20260718.txt").write_text(f"{user_num}\t{item_num}\n", encoding="utf-8")
        write_pair_file(out_dir / f"{stage}_20260718.txt", pairs, sep="\t")
        ...
        for sample_index in tqdm(range(5), desc=f"MBGCN {stage} sampling", leave=False):
            with open(sample_dir / f"sample_{sample_index}_20260718.txt", "w", encoding="utf-8") as handle:
