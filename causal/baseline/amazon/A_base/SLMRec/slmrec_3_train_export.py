#!/usr/bin/env python3
"""Train one SLMRec model per Amazon Purchase outcome and export audited scores."""
...
def main() -> None:
    ...
    visual = np.load(cache / "v_feat_20260718.npy")
    text = np.load(cache / "t_feat_20260718.npy")
    audio = np.load(cache / "a_feat_20260718.npy")
    ...
    for stage in STAGES:
        ...
        dataset = PairDataset(cache / f"train_{stage}_20260718.txt", items, seed)
    ...
    write_json(diagnostics, baseline_data_dir(root) / "SLMRec_training_diagnostics_20260718.json", indent=2)
